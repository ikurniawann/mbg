...
export default function Modul2Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed top-0 left-0 h-full z-50 w-56 bg-gradient-to-b from-red-800 to-red-700 text-white flex flex-col shadow-lg
      ...
      `}>
        <div className="p-4 border-b border-red-600">
          ...
          <p className="text-xs text-red-200 mt-1">Menu & Gizi</p>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition ${
                  isActive ? 'bg-red-600 font-medium' : 'hover:bg-red-600/70'
                }`}
              >
                <span>{item.icon}</span>
      ...
      <main className="flex-1 lg:ml-56">
        {/* Mobile Header */}
        <div className="lg:hidden flex items-center gap-3 p-4 bg-red-800 text-white">
          <button
          ...
          <span className="font-bold">Modul 2 - Menu & Gizi</span>
        </div>
        {children}
      </main>
    </div>
